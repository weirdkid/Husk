...
class ChatManager: ObservableObject {
    ...
    private func evaluateConversationTitle(for conversation: Conversation, at userTurn: Int) async {
        ...
        #if DEBUG
        print("[CompanionTitle] checkpoint=\(userTurn), currentTitle=\(conversationToUpdate.title.debugDescription)")
        #endif
        ...
        do {
            ...
            #if DEBUG
            print("[CompanionTitle] rawResponse=\(generatedTitle.debugDescription)")
            #endif
            ...
            guard !generatedTitle.contains("\n") else {
                #if DEBUG
                print("[CompanionTitle] normalizedResponse=\(generatedTitle.debugDescription)")
                print("[CompanionTitle] decision=rejected_multiline")
                #endif
                return
            }
            ...
            guard !generatedTitle.isEmpty else {
                #if DEBUG
                print("[CompanionTitle] decision=rejected_empty")
                #endif
                return
            }
            ...
            guard generatedTitle.localizedCaseInsensitiveCompare("NO_CHANGE") != .orderedSame else {
                #if DEBUG
                print("[CompanionTitle] decision=no_change")
                #endif
                return
            }
            ...
            guard generatedTitle.rangeOfCharacter(from: .alphanumerics) != nil else {
                #if DEBUG
                print("[CompanionTitle] decision=rejected_non_alphanumeric")
                #endif
                return
            }
            ...
            guard generatedTitle.count <= 60 else {
                #if DEBUG
                print("[CompanionTitle] decision=rejected_too_long length=\(generatedTitle.count)")
                #endif
                return
            }
            ...
            if !conversationToUpdate.titleWasManuallyEdited {
                ...
                saveContext()
                #if DEBUG
                print("[CompanionTitle] decision=replaced, newTitle=\(generatedTitle.debugDescription)")
                #endif
            } else {
                #if DEBUG
                print("[CompanionTitle] decision=ignored_manual_edit")
                #endif
            }
        } catch {
            print("[CompanionTitle] generation_failed error=\(error.localizedDescription)")
        }
    }
